...
def plot_hspice_lis(filename):
    ...
    while i < len(lines):
        ...
        if in_table:
            if line.startswith('time'):
                # On force en minuscules pour faciliter la recherche ensuite
    ...
    if "wen" in signals: ax1.plot(time_data, signals["wen"], label="WEN (Write=1, Read=0)", color="purple")
    ...
    # Graphique 2 : Bitlines, Entrée et Sortie
    ...
    # Graphique 3 : Nœuds Internes de la Cellule 6T SRAM
    # Internal nodes of the 6T SRAM cell
    # The exact name depends on how HSPICE prints it; handle possible cases
