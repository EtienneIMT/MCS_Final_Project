def generate_top(filename="sp_files/Final_Project_Top.sp"):
    with open(filename, "w") as f:
        ...
        f.write("Mp1 OUT IN VDD VDD pmos_sram W=864n L=20n\n")
        f.write("Mn1 OUT IN GND GND nmos_sram W=432n L=20n\n")
        f.write(".ends\n\n")
        ...
        # 2.5 Smallest Buffer
        f.write("* --- Smallest Buffer ---\n")
        f.write(".subckt BUFFER IN OUT\n")
        f.write("Xinv1 IN node1 INV\n")
        f.write("Xinv2 node1 OUT INV\n")
        f.write(".ends\n\n")
        ...
        f.write("* SAEN is active only during READ (WEN=1) AND delayed CLK=1\n")
        f.write("Xnand1 clk_del WEN saen_b NAND2\n")
        ...
        f.write("* --- Input Buffers (Prevent Infinite Driving Ability) ---\n")
        f.write("X_buf_clk CLK_ideal CLK BUFFER\n")
        f.write("X_buf_wen WEN_ideal WEN BUFFER\n")
        for i in range(7):
            f.write(f"X_buf_a{i} A_ideal_{i} A_{i} BUFFER\n")
        for i in range(16):
            f.write(f"X_buf_d{i} D_ideal_{i} D_{i} BUFFER\n\n")

        f.write("* --- TEST STIMULI (4 Cycles) ---\n")
        f.write("* 1GHz Clock (1ns period, 50ps rise/fall as per spec)\n")
        f.write("Vclk CLK_ideal 0 PULSE(0 0.7 0 50p 50p 0.45n 1n)\n\n")
        
        f.write("* Cycle 1 (0-1ns): Write '1's to Address 0 (Row 0, Col 0)\n")
        f.write("* Cycle 2 (1-2ns): Write '0's to Address 1 (Row 0, Col 16)\n")
        f.write("* Cycle 3 (2-3ns): Read Address 0 (Expect '1's)\n")
        f.write("* Cycle 4 (3-4ns): Read Address 1 (Expect '0's)\n\n")
        
        # Write Enable (0 during 0-2ns for writes, 0.7 during 2-4ns for reads)
        f.write("Vwen WEN_ideal 0 PWL(0 0  1.9n 0  1.95n 0.7  4n 0.7)\n")
        
        # Address 0 toggles at 1ns (selects between Col 0 and Col 16)
        f.write("Va0 A_ideal_0 0 PWL(0 0  0.9n 0  0.95n 0.7  1.9n 0.7  1.95n 0  2.9n 0  2.95n 0.7  4.5n 0.7)\n")
        # Row Address stays 0 (A_1 to A_6 = 0)
        for i in range(1, 7):
            f.write(f"Va{i} A_ideal_{i} 0 DC 0V\n")
            
        # Data Inputs (Write 1 in Cycle 1, Write 0 in Cycle 2)
        for i in range(16):
            f.write(f"Vd{i} D_ideal_{i} 0 PWL(0 0.7  0.9n 0.7  0.95n 0  4n 0)\n")
        ...
        # 3. Initialize SRAM cells (Address 0 stores '0', Address 1 stores '1')
        for r in range(64):
            for c in range(32):
                if c >= 16:
                    f.write(f".ic v(X_MEM.X_cell_{r}_{c}.q)=0.7v v(X_MEM.X_cell_{r}_{c}.qb)=0v\n")
                else:
        ...
        f.write(".measure tran t_write1 trig v(WL_0) val=0.35 rise=1 targ v(X_MEM.X_cell_0_0.q) val=0.35 rise=1 from=0n to=1n\n")
        f.write(".measure tran t_write0 trig v(WL_0) val=0.35 rise=1 targ v(X_MEM.X_cell_0_16.q) val=0.35 fall=1 from=1n to=2n\n")
        f.write(".measure tran t_AA_read1 trig v(WL_0) val=0.35 rise=1 targ v(Q_b_0) val=0.35 fall=1 from=2n to=3n\n")
        f.write(".measure tran t_AA_read0 trig v(WL_0) val=0.35 rise=1 targ v(Q_0) val=0.35 fall=1 from=3n to=4n\n")
        f.write(".measure tran I_avg avg I(Vdd) from=0n to=4n\n")
        f.write(".measure tran I_leakage avg I(Vdd) from=4.0n to=4.5n\n\n")
